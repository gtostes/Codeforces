...
int main(){
  ...
  for(int i=n-1; i>=0; i--){
    if(!a[i]) dp[0][i] = sm[0] + 1;
    if(!a[i]) dp[1][i] = sm[3] + 1;
    if(a[i]) dp[2][i] = sm[1] + 1;
    if(a[i]) dp[3][i] = sm[2] + 1;
    // cout << i _ dp[0][i] _ dp[1][i] _ dp[2][i] _ dp[3][i] << endl;
    for(int j=0; j<4; j++) sm[j] = (sm[j] + dp[j][i]);
  }

  Mint val = 0;
  for(int j=0; j<4; j++) val+=sm[j];
  val -= 2*n + n*(n-1)/2;
  cout << val << endl;
}