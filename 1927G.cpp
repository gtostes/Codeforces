...
void solve(){
  ...
  for(int i=0; i<n; i++){
    for(int j=i; j<n; j++){
      if(a[i]+i-1>=j or j-a[j]+1<=i) seq[i][j] = 1;
      else{
        // if(i == 0 and j == n-1) 
        int mxk = -1, mnk = n;
        for(int k = i; k<=j; k++){
          if(k-a[k]+1<=i) mxk = k;
          if(k+a[k]-1>=j and mnk == n) mnk = k; 
        }
        // if(i == 0 and j == n-1) cout << "mnk" _ mnk _ mxk << endl;
        if(mnk < mxk) seq[i][j] = 2;
      }
    }
  }

  // cout << "A" _ seq[0][n-1] << endl;

  vi dp(n,n);
  for(int i=0; i<n; i++){
    dp[i] = seq[0][i];
    for(int j=1; j<=i; j++){
      dp[i] = min(dp[i], dp[j-1] + seq[j][i]);
    }
  }
