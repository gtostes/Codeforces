...
int main(){
  ...
  vl exp1(n+1,0), exp2(n+1,0);
  exp1[0] = 1;
  exp2[0] = 1;
  for(int i=1; i<=n; i++) exp1[i] = (exp1[i-1]*27) % p1;
  for(int i=1; i<=n; i++) exp2[i] = (exp2[i-1]*27) % p2;
  ...
  for(int i=0; i<n; i++){
    ...
    ll h1 = 0, h2 = 0;
    for(int j=0; j<ss; j++){
      h1 = (h1 + exp1[j]*(s[i][j] - 'a' + 1)) % p1;
      h2 = (h2 + exp2[j]*(s[i][j] - 'a' + 1)) % p2;
      if(h1<0) h1+=p1;
      if(h2<0) h2+=p2;
      prefix[h1*p2 + h2]+=1;
    }
  ...
  for(int i=0; i<n; i++){
    ...
    for(int j = ss-1; j>=0; j--){
      h1 = (h1 + exp1[ss-1-j]*(s[i][j] - 'a' + 1)) % p1;
      h2 = (h2 + exp2[ss-1-j]*(s[i][j] - 'a' + 1)) % p2;
      if(h1<0) h1+=p1;
      if(h2<0) h2+=p2;
      ans -= 2*prefix[h1*p2 + h2];
    }
  }
