...
// ll 
// dp[i][sum][x] = dp[i-1][sum-x][0] + ... + dp[i-1][sum-x][x-1].
...
int main(){
  ...
  for(int i=1; i<36; i++){
    ...
    for(int sum = 0; sum<=500; sum++){
      for(int lst = 1; lst<=sum; lst++){
        adp[sum][lst] = hp[sum-lst][min(lst-1,sum-lst)];
        adp[sum][lst] += adp[sum][lst-1];
        if(sum == n and lst == n) ans += adp[sum][lst]; 
      }
    }

    for(int sum = 0; sum<=500; sum++) for(int lst = 0; lst<=500; lst++) hp[sum][lst] = adp[sum][lst];
  }
