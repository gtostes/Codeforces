...
int main(){
  ...
  // for(int i=1; i<=5000; i++) for(int j=1; j<=5000; j++) r[i].pb(j);

  while(1){
    int len; cin >> len;
    if(len==-1) break;
    int n; cin >> n;
    unordered_map < int, int > cset, dif;
    unordered_map < int, vi > kset;

    int res = n;
    for(int i=0; i<n; i++){
      int x,y; cin >> x >> y; string s; cin >> s; y++;
      if(res!=n) continue;
      ...
      // y++;
      if(cset[x] == cset[y]){
        if ( (dif[x]+dif[y]+b)&1 ) res = i;
      }
      else{
        if( sz(kset[cset[x]]) < sz(kset[cset[y]]) ) swap(x,y);
        auto &v = kset[cset[y]];
