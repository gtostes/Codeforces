...
int main(){
  ...
  set < int > leafs;
  for(int i=0; i<n; i++) if(lst[i] == -1) leafs.insert(i);
  V < vi > adj(n);
  // cout << "mano" << endl;

  for(int i=0; i<n-1; i++){
    // cout << i _ leafs << endl;
    int u = *leafs.begin(); leafs.erase(leafs.begin());
    // cout << i _ lst[code[i]] << endl;
    adj[u].pb(code[i]);
    adj[code[i]].pb(u);
    if(i == lst[code[i]]) leafs.insert(code[i]);
  }

  for(int i=0; i<n; i++){
    cout << i+1 << ": ";
    sort(all(adj[i]));
    for(int v: adj[i]) cout << v+1 << ' ';
    cout << endl; 
  }
