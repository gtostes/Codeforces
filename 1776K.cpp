...
int main(){
  ...
  for(int i=0; i<m; i++){
    ...
    for(int j=1; j<=e; j++){
      ...
      // if(i==0) cout << i _ add << endl;
      for(int k=0; k<m; k++){
        if(k==i) continue;
        // if(i==0 and j==2) cout << "other" _ k _ other[k] << endl;
        add*=other[k];

        other[k]-=chance[n-s[k]][j];
      }
      ans+=add;
      // if(i==0) cout << i _ ans << endl;
    }
    cout << ans << ' ';
  }
  cout << endl;
