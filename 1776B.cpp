...
int main(){
  ...
  vl a(n); each(y,a) cin >> y;

  for(int l=2; l<=n; l++){
    for(int i=0; i+l-1<n; i++){
      ans[i][i+l-1] = ans[i][i] + ans[i+1][i+l-1];
      for(int j=i; j<i+l-1; j++){
        ans[i][i+l-1] = min(ans[i][i+l-1], ans[i][j] + ans[j+1][i+l-1]);
      }
      ans[i][i+l-1] += min((a[i+l-1]-a[i]-1)/2,h);
    }
  }


  cout << ans[0][n-1] + h << endl;
}